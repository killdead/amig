(function($) {
    window.bookly = function(Options) {
       ...
       // Options.skip_steps.service = 1;
        ...
        else if (Options.status.booking == 'savesession') {
            saveSession();
                ...
                success     : function (response) {
                    if (response.success) {
                      ...
                      //  jQuery('.bookly-form').spin();
                        ...
                        function setSelectService($select, data, value) {
                            // reset select
                            $('option:not([value=""])', $select).remove();
                            // and fill the new data
                            var docFragment = document.createDocumentFragment();
                            ...
                            $.each(data, function(key, object) {
                                ...
                                option.value = object.id;
                                option.text = object.name;
                                services.push( object.id);
                                docFragment.appendChild(option);
                            });

                            var option = document.createElement('option');
                            option.value = services.join();
                            option.text = 'Tous les services';
                            docFragment.appendChild(option);
                        ...
                        function setSelects($chain_item, location_id, category_id, service_id, staff_id) {
                            ...
                            setSelectService($chain_item.find('.bookly-js-select-service'), _services, service_id);
                        ...
                        setSelectService($select_service,  services);
                        ...
                        $next_step.on('click', function (e) {
                            ...
                            if (stepServiceValidator()) {
                                ...
                                $('.bookly-js-chain-item:not(.bookly-js-draft)', $container).each(function () {
                                    ...
                                    var service_ids =  $('.bookly-js-select-service', $chain_item).val().split(',');
                                    if(service_ids.length > 0) {
                                        service_ids.forEach(function (item) {
                                            has_extras += services[item].has_extras;
                                        });
                                    }
                                    else {
                       ...
                       // $('.bookly-js-select-service').val(2).trigger('change');
                        $('.bookly-js-next-step', $container).click();
                    }
                } // ajax success
            }); // ajax
        }
        ...
        function stepTime(params, error_message) {
            ...
            //jQuery('.bookly-form').addClass('bookly-spin-overlay').spinner();
            xhr_render_time = $.ajax({
                ...
                success     : function (response) {
                    ...
                    var $columnizer_wrap  = $('.bookly-columnizer-wrap', $container),
                        ...
                        salles_active_class = response.salles_active_class,
                        salle1 = response.salle1,
                        salle2 = response.salle2
                    ;
                    // 'BACK' button.
                    ...
                    $('.bookly-calendar-trigger').on('click', function(e) {
                        $('.bookly-slot-calendar').slideToggle();
                    });
                   ...
                   // }
                    ...
                    if (response.has_slots) {
                        ...
                        if (columns_per_screen > 8) {
                            columns_per_screen = 8;
                      ...
                      //  columns_per_screen = 8;
                      //   console.log(columns_per_screen);
                        ...
                        $time_next_button.on('click', function (e) {
                            ...
                            if ($screens.eq(screen_index + 1).length) {
                              //  $screens.eq(screen_index-1).hide();
                               // $screens.eq(screen_index+1).hide();
                               // console.log('screen width:'+(Options.is_rtl ? '+' : '-') + ( screen_index + 1 ) * $current_screen.width());
                                $columnizer.animate(
                                ...
                                //$current_screen.hide();
                                $current_screen = $screens.eq(++ screen_index).show().addClass('active');
                                $current_screen.siblings().removeClass('active');
                                ...
                                if (screen_index + 1 == $screens.length && !has_more_slots) {
                                  ...
                                  $("span.startweek").text($('.bookly-day',$('.bookly-time-screen.active')).first().data('datelong'));
                                  $("span.endweek").text($('.bookly-day',$('.bookly-time-screen.active')).last().data('datelong'));
                        ...
                        jQuery('.bookly-form').removeClass('bookly-spin-overlay');
                    }
                    ...
                    function initSlots() {
                      //  console.log(active_staffs);
                        var $buttons    = $('> button', $columnizer),
                            ...
                            $column2,
                            $screen,
                            $firtbuttonofcolumn = false,
                            columnSlots = {
                                "10 h 00": $('<div><span></span></div>').addClass('bookly-hour booked'),
                                "12 h 00": $('<div><span></span></div>').addClass('bookly-hour booked'),
                                "14 h 00": $('<div><span></span></div>').addClass('bookly-hour booked'),
                                "16 h 00": $('<div><span></span></div>').addClass('bookly-hour booked'),
                                "18 h 00": $('<div><span></span></div>').addClass('bookly-hour booked'),
                                "20 h 00": $('<div><span></span></div>').addClass('bookly-hour booked'),
                                "22 h 00": $('<div><span></span></div>').addClass('bookly-hour booked')
                            },
                            column1Slots = jQuery.extend({}, columnSlots),
                            column2Slots = jQuery.extend({}, columnSlots),
                            column1Slotslength = 0,
                            column2Slotslength = 0,
                            bookday = "first",$button_bookday = null;


                        var column_active = salles_active_class;
                        if (show_day_per_column) {
                            ...
                            while ($buttons.length > 0) {
                                ...
                                if ($buttons.eq(0).hasClass('bookly-day')) {
                                    ...
                                    if(bookday != 'first') {
                                        ...
                                        $column = $('<div>').addClass('bookly-column '+column_active);
                                        $column1 = $('<div>').addClass('bookly-column-1');
                                        $column2 = $('<div>').addClass('bookly-column-2');
                                        column1Slotslength = column1Slots.length;
                                        column2Slotslength = column2Slots.length;

                                        for(var key in column1Slots)
                                        {
                                     //      console.log('line 1225',key,column1Slots[key].data());

                                            $column1.append(column1Slots[key].clone());
                                            column1Slots[key].remove();
                                        }


                                        for(var key in column2Slots)
                                        {
                                  //          console.log('line 1234',key,column2Slots[key].data());
                                            $column2.append(column2Slots[key].clone());
                                            column2Slots[key].remove();
                                        }

                                        $column.append($button_bookday.clone(),
                                           $('<div></div>').clone().addClass('colsalle salle1'),
                                            $('<div></div>').clone().addClass('colsalle salle2'),
                                            $column1,$column2);
                                        $button_bookday.remove();
                                     ...
                                     //
                                      //  $column.append($column2);
                                        ...
                                        /** INITIALISE ColumnSlots 1 and 2 **/
                                        column1Slots = jQuery.extend({}, columnSlots);
                                        column2Slots = jQuery.extend({}, columnSlots);

                                            slots_count = 1;
                                        /** CREATE Column for Bookday **/

                                        $button_bookday = $($buttons.splice(0, 1));
                                        $button_bookday.addClass('bookly-js-first-child');
                                        bookday = $button_bookday.val();
                                    console.group('line 1217','Bookly-day '+bookday);
                                        $firtbuttonofcolumn = true;
                                    ...
                                    var salle_id = $button.data('salleid');
                                    var staff_hour = $button.data('hour');
                                    var staff_hour_available = $button.data('available');
                                    console.log('line 1277',$button.data(),salle_id,salle1,staff_hour);
                                    if( 1 == staff_hour_available) {
                                        if (salle_id == salle1) {
                                           // if(staff_hour == '20 h 00')
                                                column1Slots[staff_hour] = $button.clone();
                                        }

                                        if (salle_id == salle2) {
                                          //  column2Slots[staff_hour] = $button.clone();
                                           // if(staff_hour == '18 h 00')
                                                column2Slots[staff_hour] = $button.clone();
                                        }
                                    }
                                    $button.remove();
                            ...
                            for(var key in column1Slots)
                            {
                                // console.log('line 1298',key,column1Slots[key].data(),column1Slots[key]);
                            ...
                            $column.append($button_bookday.clone());
                            $column.append($('<div></div>').clone().addClass('colsalle salle1'));
                            $column.append($('<div></div>').clone().addClass('colsalle salle2'));
                        ...
                        while (has_more_slots ? $columns.length >= columns_per_screen : $columns.length) {
                            $screen = $('<div class="bookly-time-screen"/>').addClass('active');
                            for (i = 0; i < columns_per_screen; ++i) {
                        ...
                        $('button.bookly-hour', $container).off('click').on('click', function (e) {
                            ...
                            staff_ids.push($this.data('staff'));
                            var services = JSON.parse($this.val());
                            chain[0] = {
                            ...
                            $.ajax({
                                ...
                                crossDomain : 'withCredentials' in new XMLHttpRequest(),
                                success     : function (response) {
                                    $('#book-popup-title .salle').text($this.data('salle'));
                                    $('#book-popup-title .creneau').html($this.data('creneautxt'));
                                    $('#book-popup-date').text($this.data('datetxt')+ ' à ' + $this.data('hour'));
                                    $('#book-service').val(services[0][0]);
                                    $('#book-staff').val(staff_ids);
                                    $('#book-nop').val(3);
                                    $('.book-popup-select-pers').hide();
                                    var the_select = $('#book-popup-select-pers-salle-'+$this.data('salleid')+'-'+$this.data('creneautype'));

                                    the_select.val(
                                        the_select.find('option:first').val()
                                    );
                                    $('#book-slot').val($this.val());
                                    the_select.trigger('change');
                                    the_select.show();
                                    $('#fancybox').click();
                                }
                            });
                        ...
                        $('.bookly-time-step', $container).width((columns_per_screen+1)* column_width);
                    ...
                    $('.book-popup-select-pers').on('change',function(e) {

                        //   $('#book-popup-price').html($( "option:selected",$(this)).text());
                        var val = ($(this).val()).split(',');

                        $('#book-nop').val(val[0]);
                        $('#book-service').val(val[1]);
                        $('#book-staff').val(val[2]);
                        var $slot = $('#book-slot').val();
                        var slot = JSON.parse($slot);
                        slot[0][0] = val[1];
                        slot[0][1] = val[2];
                        $('#book-slot').val(JSON.stringify(slot));

                        // $('#book-variation-id').val(val[1]);

                    });

                    $('#book-popup-cancel').on('click',function() {
                        $.fancybox.close();
                        Ladda.stopAll();
                    });
                }
        ...
        function saveSession() {
            ...
            var chain = {};
            chain[0] = {
                service_id        :  $('#book-service').val(),
                staff_ids         :  $('#book-staff').val(),
                number_of_persons :  $('#book-nop').val(),
                quantity          : 1
            };
            var slot = $('#book-slot').val();
