...
class Frontend
{
    ...
    //	die('origine Frontend');
        ...
        $this->customerProfileController = \Bookly\Frontend\Modules\CustomerProfile\Controller::getInstance();
        $this->wooCommerceController     = \Bookly\Frontend\Modules\WooCommerce\Controller::getInstance();
        if ( Lib\Config::isPaymentTypeEnabled( Lib\Entities\Payment::TYPE_MOLLIE ) ) {
            $this->mollieController = \Bookly\Frontend\Modules\Mollie\Controller::getInstance();
        }
        if ( Lib\Config::isPaymentTypeEnabled( Lib\Entities\Payment::TYPE_PAYPAL ) ) {
            $this->paypalController = \Bookly\Frontend\Modules\Paypal\Controller::getInstance();
        }
        if ( Lib\Config::isPaymentTypeEnabled( Lib\Entities\Payment::TYPE_PAYSON ) ) {
            $this->paysonController = \Bookly\Frontend\Modules\Payson\Controller::getInstance();
        }
        if ( Lib\Config::isPaymentTypeEnabled( Lib\Entities\Payment::TYPE_PAYULATAM ) ) {
            $this->payulatamController = \Bookly\Frontend\Modules\PayuLatam\Controller::getInstance();
        }
        if ( Lib\Config::isPaymentTypeEnabled( Lib\Entities\Payment::TYPE_2CHECKOUT ) ) {
            $this->twocheckoutController = \Bookly\Frontend\Modules\TwoCheckout\Controller::getInstance();
        }
        // Register shortcodes.
